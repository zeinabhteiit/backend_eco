import {
  ...
  export function createNewOrderShipment(req, res) {
    const data = req.body;
    createOrderShipment(data).then(result => {
      res.status(200).json({ data: result, message: "Shipment created successfully", error: null });
    }).catch(err => {
  ...
  export function getShipment(req, res) {
    ...
    getShipmentById(id).then(result => {
      if (!result) {
  ...
  export function updateOrderShipment(req, res) {
    const id = req.params.id;
    const data = req.body;
    updateShipment(id, data).then(result => {
      res.status(200).json({ data: result, message: "Shipment updated successfully", error: null });
    }).catch(err => {
  ...
  export function deleteOrderShipment(req, res) {
    const id = req.params.id;
    deleteShipment(id).then(result => {
      res.status(200).json({ data: result, message: "Shipment deleted successfully", error: null });
    }).catch(err => {
